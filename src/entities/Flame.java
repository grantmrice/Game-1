...
public class Flame extends Particle{
	...
	public Flame(Level level, Entity owner, int x, int y, int centerX, int centerY, int color){
	...
	public Flame(Level level, Entity owner, int x, int y, int centerX, int centerY, int color, double speed, int duration, boolean fade){
		super(level, owner, x, y, color);
		angle = Math.sqrt(Math.pow(x-centerX,2)+Math.pow(y-centerY,2)) == 0 ? Math.random()*2*Math.PI : Math.atan2(y-centerY, x-centerX);
		init();
		this.speed = speed;
	...
	public void tick(){
		if(owner.state == -1 || color < 0 || tickCount >= duration) state = -1;
